...
pkgs <- unique(rbind(
  sna4tutti:::tutorial_package_requirements(),
  sna4tutti:::tutorial_package_requirements(
    fields = "Suggests",
    packages = c("ndtv", "networkDynamic")
  )
))
...
snafun::v_betweenness(classroom)
```
...
-   a `data.frame` that has the onset and terminus values for each
    ...
    vertex attributes
-   a `data.frame` that contains the times at which edges (*edge
...
construct such a network in the future, you can consult the help for the function in
R. Also, the instructors of this course can assist you. 
