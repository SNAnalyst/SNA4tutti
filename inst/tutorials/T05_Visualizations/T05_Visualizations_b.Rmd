...
```{r test_DataLoad, results='asis'}
 net <- SNA4DSData::DSstudents

if (table(igraph::V(net)$name == c(paste0("S", 1:81))) != 81) {
...
THIS TEST SHOULD YIELD A MESSAGE

```{r test_DataLoad2, results='asis'}

# exercise=FALSE,

# echo=TRUE, include=FALSE, exercise=TRUE, exercise.setup ="dataload"

net <- SNA4DSData::DSstudents

if (table(igraph::V(net)$name == c(paste0("S", 1:81))) != 8100) {
  sna4tutti::broken_info()
} else {
  cat(" ")
}
...
```{r manip, echo=TRUE, exercise=TRUE, exercise.setup ="dataload"}
net <- igraph::delete.vertices(net, v = 1:10)
```


<!-- ```{r after_manip} -->
<!-- browser() -->
<!-- print(igraph::ecount(net)) -->
<!-- ``` -->
...
```{r check_results, echo=FALSE, results='asis'}
cat("\n\n\n\n\n\n")

if (length(broken_list) > 0) {
  cat("There were issues with the following chunk(s):\n\n")
  print(unlist(broken_list))
  cat("\n\n")
