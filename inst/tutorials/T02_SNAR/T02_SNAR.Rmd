...
<!-- <p style="font-size:0.8rem; font-style:italic; text-align:right"> -->
<!-- (Photo by [Greg Rosenke](https://unsplash.com/@greg_rosenke) on -->
<!-- [Unsplash](https://unsplash.com/s/photos/tool)) -->
<!-- </p> -->
...
<div class = "green">The good news first</div><br>
...
<div class="red">Then the bad news</div><br>
...
<div class="orange">Some better news</div><br>
...
<div class = "green">All is well that ends well</div><br>
...
We have tried to use a consistent API for the `snafun` package. The package 
is very recent (we developed it over the last few months), so there may be 
some space for further development and extension. But this is the general idea 
behind the API. This is what it looks like:
...
Anyway, try to remember the API above, so you can easily find the functions you need.
And, most of all: ***ENJOY!!!***

![](images/gratisography-sunburn-man-free-stock-photo-1170x780.jpg){width=70}
...
receiver), there are ni *loops* (a loop is an edge from an actor to himself).
...
info on what these attrbutes mean.
...
Now, here is a little challenge.: - read in the network called "Texas", that is
inside the `emon` data - apply the functions from above on the Texas network
instead of on Cheyenne - convert the Texas network to an `igraph` object, call
that `texas_i` for now. - determine all of these numbers for `Texas_i`. - Check
if you get the same results (you should)
...
have it, ask Claudia and Roger and it might just magically be added to the
package!]
...
Actually, there is a shorter way of doing this, because `snafun` actually 
has a specific function to add vertex names:
...
`snafun::add_vertex_attributes` to set any other vertex attribute), because 
the `igraph` and `network` objects store vertex names differently.
Our specialized function does it correctly for both graph classes, so you don't 
have to remember how they differ exactly. Much easier, and much safer too.
...
(there are alternatives, you can "Kerrville Police Department" replace with
...
infinitely large. Whhy? Because there is not a single tie from the Salvation
Army to any other organization. There are some ties *to* the Salvation Army, so
they are not completely alone in the end.
...
analyses of social networks. You need to have ***mastered the foundations*** of how
to deal with this type of data before you can start doing cool stuff with them.
...
:-------------------------:|:-------------------------:
![](images/foundation_politico.png){width=90%} |  ![](images/pexels-george-milton-6954109_smaller.jpg){width=90%}
...
It's going to go faster from here and we'll be more adventurous in every 
future tutorial and lab and you now have the skills to travel along.
