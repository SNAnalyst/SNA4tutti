...
Every statistics in the result's table allows us to reject the null hypothesis of no effects. The coefficients produced by the `nodematch` term inform us of the existence of homophilic effects in each grade. Grades 10 and  12 are the two categories where these effects are less likely to take place. Still, the probability of observing homophily in those years is very high. The problem with these two categories is their statistical significance. According to the alpha that we choose, we might conclude that the effect happened by chance. 
...
The plot simply visualizes these results using box plots. The blue square and the boxplots display the simulated data, and the black tick line represents the observed values. We want the black tick line to be as close as possible to the blue squares and possibly never outside the min and max interval. 
...
Checking on the model summary above, we see that the configurations that are highly represented in this sample are 012, 102, 021D, 021U, 201, 111D. The term 
...
This time our results seem to be more interesting. Either ways, you'll get an error message. Obviously, we cannot be sure since we did not check the MCMC diagnostics and the goodness of fit. If the model fit is good, what do these results mean for the Sampson network? 
...
Final note: for making the tutorial work faster, the MCMC burn in and sample size have been set to very low standards. In order to have better model (better diagnostics), remember to increase these numbers in a real data analysis.
