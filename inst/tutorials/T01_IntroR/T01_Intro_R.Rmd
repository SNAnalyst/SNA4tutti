---
title: "T01 - Intro to R"
description: "An Introductory tutorial for programming in the R language"
output: 
  learnr::tutorial:
    fig_caption: no
    progressive: true
...
Welcome to this tutorial! `r rproj()` is a programming language and free software
environment for statistical computing and graphics supported by the R
Core Team and the `r rproj()` Foundation for Statistical Computing.

Even if you are excellent at programming, if you haven't used `r rproj()` before
...
![](images/programmer.png){width=75%}

## Checking installation

### R installation

You need to have installed `r rproj()` version 4.3.1 (or higher) and this tutorial
...
You need to have installed RStudio version 2023.6.1.524 or above. Let's
...
<br>[the result will appear underneath the code box]
...
If you want `r rproj()` to print it while you specify it, you use round brackets
...
check what are a and b below. Can you check whether
...
```{r test_grade_code4, results = 'markup'}

a <- 3
b <- 5


if (base::isFALSE((a != b) & ((a + b) == 8))) {
  broken_info()
...
work. In `r rproj()` you comment using \#
...
In `r rproj()` indentation is important to keep your code readable, but it is not
crucial. It will be running even if you write it in the most terrible
way. `r rproj()` uses curly braces and that's what matters the most.

***Just please DON'T!***

R studio will automatically indent your code if you select it and click
CRTL I. Try to write some messed up code and to indent it afterward. (NOTE: 
this only works inside RStudio, not inside this tutorial environment).
...
to do it if you decide to write a package in `r rproj()` while using `r rproj()` for data
...
To make the best of the `r rproj()` language, you'll need a strong understanding
of the basic data types and how to operate on them.

-   *character*: "a", "swc"
-   *numeric*: 2, 15.5
-   *integer*: 2L (the L tells `r rproj()` to store this as an integer)
-   *logical*: TRUE, FALSE
...
```{r test_grade_code6, results = 'markup'}
...
if (base::isFALSE(base::table(c("not spicy", "a bit spicy", "mildly spicy", "very spicy", "Waw! Fire!") == base::levels(grades)) == 5)) {
  broken_info()
  error <- knitr::opts_current$get(name = "label")
  errors <- base::append(errors, error)
}
...
you still don't know how to find help in `r rproj()`. You needed to know that
...
information you need will pop up on your `r rproj()` studio window (or open on
browser)
...
-   NA: (Not Available) is a missing value indicator. It suggests that a
    value should be present but is unknown. `r rproj()` considers NA as logical.
...
There are 5 data structures in `r rproj()`:
...
can create one in `r rproj()` with the function `data.frame()`. Every column needs
...
```{r test_grade_code8, results = 'markup'}
...
df1 <- data.frame(rbind(a, b))
colnames(df1) <- c('col1', 'col2', 'col3', 'col4', 'col5', 'col6', 'col7', 'col8', 'col9', 'col10')


if (base::isFALSE((base::ncol(df) == 2) & (base::ncol(df1) == 10))) {
  broken_info()
  error <- knitr::opts_current$get(name = "label")
  errors <- base::append(errors, error)
}
...
A matrix in `r rproj()` is like a data frame but it can host only one type or
...
```{r test_grade_code9, results = 'markup'}
...
if (base::isFALSE(base::dim(m)[1] == 10)) {
  broken_info()
  error <- knitr::opts_current$get(name = "label")
  errors <- base::append(errors, error)
}
...
When you start to work in `r rproj()` (as in any other language), you need to set
up your working directory. In fact, you won't be able to import any data
unless `r rproj()` knows where to find it first.
...
`r rproj()` has an inbuilt function to import *.txt* data and turn it into a
data frame:

`read.table()`

`r rproj()` also has an inbuilt function to import *.csv* data and turn it into
a data frame:
...
-   *file* = you pass the file's name within quotation marks, including
    ...
    `r rproj()`, you can also omit the argument's name and write
    `read.csv("data.csv")`.
...
-   *stringsAsFactors* = if your data contains character variables, `r rproj()`
...
-   *na.strings* = if you know that you are importing a file with
    missing data, you should use this argument to help `r rproj()` to recognize
...
Hence you would import it into `r rproj()` this way
...
over time. It is possible to import excel data into `r rproj()`.

If it is a simple spreadsheet, the easiest way is to save it as a *.csv*
or *.txt* file and import it that way. Otherwise, you need to use an `r rproj()`
...
Will import into `r rproj()` the data included in the range column A row 2 and
...
that blurb you just imported into `r rproj()`. Instead of panicking (ok, you can
panic for 10 seconds), use the functions that `r rproj()` already set up for you
...
`r rproj()` does not believe in this kind of zeros (in other it does), and it
...
After all this fishing, you ABSOLUTELY deserve a fish!


![](images/5490632_1652_foto_1.jpg){width=75%}
...
Hence you tell `r rproj()` that you are interested in the `mpg` column,
...
How much better is this code?? Let's understand what happened.
...
table(mtcars$cyl)
mtcars$cyl <- ifelse(mtcars$cyl == 8, "cool", "not_cool")
...
The very last version of `r rproj()` included a helpful tool called the pipe.
...
expression to the very first part of the right-hand side expression. 
This is a bit
revolutionary for the `r rproj()` way of thinking, and it allows you to perform
...
That was quick, right? What happens is that `r rproj()` inserts *iris* as the very 
first argument of the `subset` call. In other words, your pipe actually 
runs `subset(iris, Species == "setosa")`.
...
For a short expression like this, you might as well have typed
`str(iris)` directly. The pipe is quite handy when you have a series 
of subsequent actions to perform, so: first do this, then with that result do 
this and with that result do the following and then plot a histogram. 
This is nice and creates a very readable *pipeline* of steps that is easy to 
follow and understand. You will see the pipe in the SNA4DS course in several 
places throughout the semester and in many many places on the world-wide interwebs.
...
In `r rproj()` packages are also called libraries. You can install them with the
...
Also, some packages conflict with each other, and your `r rproj()` session might
...
In `r rproj()` a function ALWAYS uses {curly braces} :)
...
Ideally you should save your function in a clean `r rproj()` script and import
...
It's a little difficult, but I KNOW THAT YOU CAN DO IT!!! Call your
...
Can you make `r rproj()` print "Win" when team 1 wins and "Lose" when team 1
loses?
...
As we have just seen, `r rproj()` uses for loops as much as any other
programming language. However, in `r rproj()`, there is an alternative to
...
The apply family is large! This is an overview of the ones you'll encounter the most:
...
## Two Best R Practices

Wow, you have done A LOT! Yes, this probably took you a few hours a go through, 
but you are on your way to R-Ninjaness!

![](images/depositphotos_64130295.jpg)

We want to close off this tutorial with some best practices. 

You already learned that it is best to indent your code appropriately, 
that you should use the `<-` assignment operator instead 
of `=`, 
and some other ways to make your code nicely useable.
...
In many code examples in books and on the internet, you see that a library/package is
loaded/attached (for the purpose of this course, we will use these terms
...
Once you save a script based on your interactive code, 
chances are it won't work well in the future, because it depends on specific
packages being loaded that may not be loaded when run your script again. 
As a result, debugging becomes a nightmare. Trust us, we have way too
much experience with this scenario ourselves...
3. When you load anything into R, it gets attached to R's "search path." 
Generally
speaking, when you attach _package1_, R places on the top of the search path.
This makes all of the functions of _package1_ fully available to you. 
Then, when you attach _package2_ later, that will then move to the front
...
```{r load_friendship, echo = FALSE, eval = TRUE}
friendship <- structure(list(mel = list(list(inl = 1L, outl = 5L, atl = list(
    na = FALSE)), list(inl = 1L, outl = 10L, atl = list(na = FALSE)), 
    list(inl = 2L, outl = 4L, atl = list(na = FALSE)), list(inl = 2L, 
        outl = 8L, atl = list(na = FALSE)), list(inl = 2L, outl = 9L, 
        atl = list(na = FALSE)), list(inl = 3L, outl = 5L, atl = list(
        na = FALSE)), list(inl = 4L, outl = 3L, atl = list(na = FALSE)), 
    list(inl = 4L, outl = 5L, atl = list(na = FALSE)), list(inl = 4L, 
        outl = 8L, atl = list(na = FALSE)), list(inl = 5L, outl = 4L, 
        atl = list(na = FALSE)), list(inl = 5L, outl = 10L, atl = list(
        na = FALSE)), list(inl = 6L, outl = 9L, atl = list(na = FALSE)), 
    list(inl = 7L, outl = 1L, atl = list(na = FALSE)), list(inl = 8L, 
        outl = 2L, atl = list(na = FALSE)), list(inl = 9L, outl = 1L, 
        atl = list(na = FALSE)), list(inl = 9L, outl = 2L, atl = list(
        na = FALSE)), list(inl = 9L, outl = 8L, atl = list(na = FALSE)), 
    list(inl = 10L, outl = 4L, atl = list(na = FALSE))), gal = list(
    n = 10, mnext = 19L, directed = TRUE, hyper = FALSE, loops = FALSE, 
    multiple = FALSE, bipartite = FALSE), val = list(list(na = FALSE, 
    vertex.names = "john", name = "john"), list(na = FALSE, vertex.names = "peter", 
    name = "peter"), list(na = FALSE, vertex.names = "baheer", 
    name = "baheer"), list(na = FALSE, vertex.names = "elise", 
    name = "elise"), list(na = FALSE, vertex.names = "felix", 
    name = "felix"), list(na = FALSE, vertex.names = "mohammad", 
    name = "mohammad"), list(na = FALSE, vertex.names = "mikel", 
    name = "mikel"), list(na = FALSE, vertex.names = "joan", 
    name = "joan"), list(na = FALSE, vertex.names = "gerrit", 
    name = "gerrit"), list(na = FALSE, vertex.names = "gideon", 
    name = "gideon")), iel = list(2:1, 5:3, 6L, 9:7, 11:10, 12L, 
    13L, 14L, 15:17, 18L), oel = list(c(15L, 13L), c(16L, 14L
), 7L, c(3L, 18L, 10L), c(1L, 6L, 8L), integer(0), integer(0), 
    c(4L, 9L, 17L), c(5L, 12L), c(2L, 11L))), class = "network")
```



```{r test_load_friendship, results = 'markup'}
friendship <- structure(list(mel = list(list(inl = 1L, outl = 5L, atl = list(
    na = FALSE)), list(inl = 1L, outl = 10L, atl = list(na = FALSE)), 
    list(inl = 2L, outl = 4L, atl = list(na = FALSE)), list(inl = 2L, 
        outl = 8L, atl = list(na = FALSE)), list(inl = 2L, outl = 9L, 
...
if (!snafun::is_network(friendship) | !snafun::count_edges(friendship) == 18) {
...
You have a network in your R session that is called `friendship`. This is a 
network and looks like this. 


```{r opar_1a, echo = FALSE, eval = TRUE}
opar <- par("mar")
par(mar = c(1, 1, 1, 1))
```


```{r plot_load_friendship, exercise.setup = "load_friendship", exercise = TRUE}
par(mar = c(1, 1, 1, 1))
network::plot.network(friendship, 
  label = snafun::extract_vertex_names(friendship),
  vertex.lwd = 3,
  label.cex = 1,
  vertex.cex = 3,
  edge.lwd = 3,
  vertex.col = c(2 ,1 ,3 ,3 ,3 ,1 ,2 ,1 ,1 ,3) + 1
)
```

You load the `sna` package to calculate the betweenness scores of the actors in
the network (no worries, soon we will discuss the betweenness measure in
detail). Run the following and see what happens:


```{r library1, message = FALSE, warning = FALSE, exercise = TRUE, exercise.setup = "load_friendship"}
library(sna)
betweenness(friendship)
```

Nice. You don't know yet what this means, but it looks meaningful.<br>

Now, let's say you loaded the `igraph` package, as part of your interactive 
analysis of the data. Then, after half an hour, you realize you need the 
betweenness scores again of 
the persons on this network, so you run again `betweenness(friendship)`. This 
is what you get:

```{r error, exercise = TRUE, exercise.setup = "load_friendship"}
library(igraph)
# half an hour of interesting analyses
# what again were those betweenness scores? Let me check them again:
betweenness(friendship)
```
...
What happened? Well, just like the `sna` package, `igraph` also has a function that is called
`betweenness` and since you loaded `igraph` last, the `betweenness` function
of the `igraph` package is now higher in R's search path than the
`betweenness` function of the `sna` package. 
As a result, R uses the `betweenness` function from the `igraph` package. 
But that causes an error because `igraph` expects
an object in a different form than the object `friendship`. 

Of course, it is possible that `igraph` would have been able to deal with this 
`friendship` object. In that case, you not have received an error. 
This is even worse, because now you think all is working well. 
If you do not get an error, **you may actually get a different result than you expected.** 
But, if this is part of a longer analysis pipeline, you may not notice and hence 
report results that are incorrect/do not mean what you think they do. 
...
```{r prefix, warning = FALSE, message = FALSE, exercise = TRUE, exercise.setup = "load_friendship"}
library(sna, quietly = TRUE)
library(igraph, quietly = TRUE)
sna::betweenness(friendship)
```

You now ensure that you run `betweenness` from `sna` on the `friendship` object, 
regardless of any other packages that you might have currently loaded. 
In fact, stop attaching at all!
...
*closeness* scores on this friendship network using the `sna` package. 
The appropriate function is called `closeness`. 
As just agreed upon, do not load the `sna` package for this.

```{r library4, exercise = TRUE, exercise.setup = "load_friendship"}
...
Note: this is actually what you do in Python too. There, you import 
a package and then use the name (or alias) of that package every time you 
use a function from that package. So, if you are familiar with Python, this
practice will be familiar to you already.
...
unnecessarily cumbersome. If you have data on friendships in a network, call it 
`friendship` and not `g` or `x` or something like that.
...
That's all for now! Hopefully you learned something helpful and had some fun! 
See you in the next tutorial!

![](images/artworks-000192822861-zqs0r2-t500x500.jpg)
