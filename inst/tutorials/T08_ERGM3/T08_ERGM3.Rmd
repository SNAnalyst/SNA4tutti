...
Since you have now mastered the basics, welcome to the advanced ERGM tutorial.

In this tutorial, we will cover two types of advanced ERGMs. First, the bipartite ERGM, which can be specified as a particular option of the regular ERGM with the `ergm` package. Second, we will learn the weighted ERGM, which requires a new package named `GERGM`, standing for Generalized Exponential Random Graph Models.

Fasten your seat belt, and let us begin.
...
From now on, when we fit a model, we place it in the context of doing actual research. To do so, we rely on the 7-step flow approach introduced in the ERGM2 Lab:

1. Literature research/ Questions & Hypotheses (based on the literature) [- data collection]
2. Data exploration + term selection
3. Running models (baseline, exploratory models, final model)
4. MCMC diagnostics of the model selected as the best
5. Goodness of fit
6. Interpreting results
7. Writing up your research and explaining how this journey went

Take a look at the image below. It guides you step by step through how to use the flow.
...
Just to be clear. A bipartite network has two sets of nodes that can be connected to each other but not within each set, as in the image below.

![](images/Simple-bipartite-graph.png){width=75%}

Even if the concept of a bipartite network is pretty straightforward, 
Setting up your network as a bipartite, ready for a bipartite ERGM with the `ergm` package, can be a bit tricky.
...
While adjacency matrices are squared, with the same items in rows and columns, incidence matrices have the first set of elements in rows and the second set in columns. Hence, if we have a bipartite network of 10 friends going to the Cinema Club four times in total, we will get an incidence matrix of 10 by 4 elements. 

You can import a bipartite network in the `network` package (which we need to run ERGMs with the `ergm` package) using both the edge list and the incidence matrix. 
...
Our first set of nodes (rows) consists of 10 kids, while our second set consists of the 4 playdates the kids go to (columns). 

Let's turn it into an object of class `network`. To do that, we need to specify that it is a bipartite, undirected network.
...
We already have three attributes stored in three vectors called "vertex.names", "Age", and "Gender". 

We simply need to include them in the `network` object we already created.

Pay attention to one thing: The attributes here refer to the kids, but the number of kids is 10, and the number of nodes in the network is 14. Hence, in order to make this work, the vectors containing the attributes will have information about the gender and age of the 10 kids, and NA stored for the play-dates, since this information does not exist for the play-dates, but the network expects a vector with 14 elements given that we have 14 nodes. If the attribute was about the play-dates, we would still have 14 elements in the vector, with the first 4 describing the play-dates and the last 10 coded as NA since an attribute for the play-dates is not applicable to kids. For example, if the play-dates could be classified as good "G" or bad "B", we would have a vector like this one. 

`playdates_quality <- c('G', 'B', 'B', 'G', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA')`
...
At this point, you might really think we are done. Are we? In fact, we are not. We need to pass an extra attribute to the network so it is crystal clear that we are dealing with a bipartite graph. 
...
As world experts in children's behavior and play-date-ology (just kidding!), we are fully aware of the literature, and we specify three hypotheses.

1. Older kids go to more playdates
2. There is homophily by gender (girls play with girls, boys play with boys)
3. Some playdates have more attendees than others (some sort of popularity effect)
...
Do you see those NAs at the end of the vectors? That's what we discussed before. Once more, they are there because the attributes make sense only for kids, not for play dates, but `network` stores information this way. Be careful to understand the data architecture of bipartite graphs in `network`; otherwise, you might get into errors that require painful debugging sessions!
...
Have you realized that you can specify the terms, both focusing on one partition or the other? Make sure you pay close attention to this!
...
Remember that the bipartite terms are different from the unimodal ones. The main difference is that you can specify them for one partition or the other, according to your research questions and hypotheses. 

Do you want to explain kids' behavior or the occurrence of play dates? You can do either or both

You can find all the terms you need using the function:
...
First, we run baseline models and dyadic independent exploratory models to get a sense of how the data will behave.
...
Now we can run dyadic dependent exploratory models. Since the dyadic dependent involves a simulation, it is computationally more intensive. To reduce computational time and improve model performance, we can try two things. 

First, we should constrain the model. For instance, in the example below, I'm asking the simulation to exclude networks with nodes having degrees lower than 1 (isolates) and larger than 14 (max N nodes) since they would be unrealistic in the current situation that we are exploring. Careful not to specify the model too much, since the simulation still needs the freedom to move.

Second, we can control the model by specifying how the simulation should run (but we already know how to do this!).
...
Let's assume we selected our best model by comparing AIC and BIC and making sure that all hypotheses are included in the model specification, and we now move on to check the MCMC diagnostics.
...
If and only if you think the model is good enough, you can move on to goodness of fit. Otherwise, go back and try a different model specification (getting back to steps 1, 2, 3).



#### Step 5

Is the goodness of fit good? If and only if you think that the model is good enough, you can move to the interpretation of results! Otherwise, go back and try a different model specification (getting back to steps 1, 2, 3).
...
Let's look at coefficients and odds ratios first.
...
The only significant term in the model is `nodematch` by gender. Interpreting the coefficient, we know that kids of the same gender tend to attend the same playdates. The extent to which kids go to a certain party (form edges) increases if the kids are of the same gender.

Interpreting the odds ratio (this is stochastic, but by exponentiating the coefficient you should get a number similar to $2.375$), we know that this effect is really strong since the OR is much larger than 1. 
...
Looking at the probabilities, we know that the probability of kids of the same gender attending the same play date is 70.4 % ($0.704*100$). Wow! This is really a strong effect, and it is in line with the OR, just easier to interpret!

Note: again, you might have slightly different probabilities since the model is stochastic!
...
Here we go with weighted networks! This kind of network is also expressed as an adjacency matrix. Still, rather than having a matrix populated by 0s and 1s, we have a matrix populated by other numbers representing weights. These weights imply that some edges might define stronger relationships than others. 

Even if any graph can have weights, with weighted ERGMs we can only analyze fully connected graphs.
...
This would work in a situation where we have three waves of friendship data that we recode as 

* friends for 1 year - weight 1
* friends for 2 years - weight 2
* friends for 3 years - weight 3

This is a count variable. The weight must be larger than 0 (non-negative), and it cannot fall between the categories it describes.

However, weight can express many other realities that need more complex definitions, such as the money export between countries measured in dollars. The `GERGM` package can handle both the count weight and the more complex cases, and for this reason, we are going to move out of the `statnet` environment for this kind of model. 
...
This is how this weighted network looks.
...
The `GERGM` package works in its own way, with its own environment; it does not rely on either `igraph` or `network` formats. 
...
Ok, let's move on with this research using our seven-step flow. 
...
Unlike the `ergm` setup, in the `GERGM` one, we can attach an attribute to the sender and receiver effects.

### Step two (Data Exploration)


Let's explore the covariates in our dataset.
...
NOTE3: This model might be too heavy to run in your tutorial, depending on your computer. If it does not run, run it directly in `RStudio`. You can load the data using the `data` function. 
...
_NOTE:_
I inserted a burn-in of 1000 and simulated 5000 networks. I used this setup to speed up computation within the tutorial. You will need to increase these numbers to something like 10k and 40k (or more) when you will use these models for real!
...
Is this model good enough? Let's examine the diagnostics!


#### Step four (MCMC diagnostics)

Even if the `GERGM` model automatically prints information about the model's quality while running, it is also possible to reprint it separately. 
...
Since this model is different from the regular ERGM, the trace plot doesn't need to be centered at 0. Instead, a black line indicates where the trace plot should be centered. Does this look good to you? 
...
Since our MCMC diagnostics aren't good enough, let's move on to the GOF to learn how to interpret it!
...
How about the results? Well, this model is pretty bad, but let's learn how to explore the results anyway, pretending it is okay!

The package already provides an inbuilt estimates plot. Red means significant, while the other colors mean the opposite. But a table with coefficients could be nice too, right? Here we go.
...
Mutual is not significant, so apparently the trade is a one-way operation. But sender and receiver as far as GDP are concerned! It seems that if we take GDP into account, we can predict the intensity of the trade relationship (the weight of the edge). In fact, here, we don't only predict the existence of an edge but also the strength of the edge. GDP seems to be positively associated with trade. Countries with higher GDP trade more, both sending and receiving goods. It makes sense, right? 

How much does GDP influence the relationships between nodes? Well, up to you to check odds ratios and probabilities to find out. I leave this to you since you are already a pro :)
...
Be ready to jump into a time machine with temporal ERGM (TERGM) in the next tutorial!
